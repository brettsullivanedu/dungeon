...
class Player(Character):
    ...
    def perform_action(self, action, target):
        """
        Performs the selected action on the target.
        """
        if action == "Fireball" and isinstance(self, Mage):
            self.cast_fireball(target)
        elif action == "Shield Bash" and isinstance(self, Warrior):
            self.shield_bash(target)
        elif action == "Stealth Attack" and isinstance(self, Rogue) and self.is_stealthed:
            self.stealth_attack(target)
        else:
            print("Invalid action or character class.")
